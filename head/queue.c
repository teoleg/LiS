 ...
 * RCS Id          : $Id: queue.c,v 9.3 2010/03/31 15:05:37 hecht Exp $
...
int lis_lockq_fcn(queue_t *q, char *file, int line)
{
    ...
    /* We don't want this to be interrupted by stray signals if we
     * in a service routine running in user context since in the
     * Adax modified version we call lis_run_queues() directly from
     * lis_setqsched() if we're not in interrupt context and can_call
     * is set.  
     *
     * We use this loop instead of lis_down_nintr because we don't want 
     * to lose the debugging info provided here.  If we used lis_down_nintr
     * then we would just lis_down_nintr for our file and line. */
    while ((ret = lis_down_fcn(&qs->qs_sem, file, line)) == -EINTR) 
	{
	/* Just prevent us from staying stuck here forever, just in case...  */ 
	if (++eintr > 0x7fffffff)
		{
		printk("lis_lockq: lis_down_fcn failed %d times at %s:%d\n",
			eintr, file, line);
		break;
		}
	}
...
#ifdef DANDEBUG
    if (eintr)
	printk("lis_lockq: lis_down_fcn failed %d times at %s:%d on q=0x%p\n",
		eintr, file, line, q);
#endif
...
rmv_msg(queue_t *q, mblk_t *mp)
{
    ...
    {					/* empty queue */
	q->q_flag |= QWANTR;	/* note that someone wants to read */
	return(mp) ;
    }
...
/* ### No KMEM-CACHE Work */
    ...
    if(rtn)
    {
        lis_mark_mem(rtn, lis_queue_name(q), MEM_MSG, BLK_MB_GETQ);
    }
#endif
...
/* ### No KMEM-CACHE Work */
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
    lis_mark_mem(mp, lis_queue_name(q), MEM_MSG, BLK_MB_PUTQ) ;
#endif
...
/* ### No KMEM-CACHE Work */
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
    lis_mark_mem(mp, lis_queue_name(q), MEM_MSG, BLK_MB_PUTBQ) ;
#endif
...
/* ### No KMEM-CACHE Work */
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
  lis_mark_mem(q, name, MEM_QUEUE, BLK_QUEUE);	/* mark memory w/queue name */  
#else
...
lis_bcanputnext_anyband(queue_t *q)
{
    ...
    /* canputnext...  go to the next queue */
    q = q->q_next;
    if (q == NULL)
	return(0);
