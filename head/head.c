 ...
 * RCS Id          : $Id: head.c,v 9.10 2013/09/11 14:04:33 hecht Exp $
...
extern unsigned long lis_debug_err[NUMBER_OF_DEBUG_COUNTERS];
#endif
...
#define CPFL(p,a,func,f,l)						\
    do {								\
	  ...
	  if (LIS_DEBUG_CP)						\
	      printk(lis_cp_fmt, pp->cntr, pp->cpu,			\
		      		(func),(f),(l),(void *)(p),(a));	\
       } while (0)
...
volatile unsigned long lis_runq_srv_rpt_cnts[LIS_NR_CPUS] ;
...
volatile unsigned long lis_setqsched_usr_cnts[LIS_NR_CPUS] ;
volatile unsigned long lis_setqsched_defer_cnts[LIS_NR_CPUS] ;
volatile unsigned long lis_setqsched_poke_cnts[LIS_NR_CPUS] ;
 ...
 */
void lis_dismantle(stdata_t *hd, cred_t *creds)
{
    queue_t	*q ;
...
lis_alloc_stdata(void)
{
    ...
    init_completion(&head->sd_close_wt);
    init_completion(&head->sd_wiocing);
    init_completion(&head->sd_wread);
    init_completion(&head->sd_wwrite);
    init_completion(&head->sd_closing);
...
copyin_msgpart( struct file *f, stdata_t *hd, mblk_t **m, char *sb, 
		int wroff, unsigned int ioctl32)
{
    strbuf_t kbuf;
    int err;
    int strbuflen;
    ...
    if (!ioctl32)
	strbuflen = sizeof(strbuf_t);
    else
	strbuflen = sizeof(strbuf32_t);

    *m = NULL ;					/* NULL msg ptr by default */
    if ((err=lis_check_umem(f,VERIFY_READ,sb, strbuflen))<0)
	return(err);

    /* fetch strbuf */
    if (!ioctl32)
    {
                if ((err = lis_copyin(f, &kbuf, sb, strbuflen)) < 0)
                {
#ifdef LIS_HAS_PROC_ENTRIES
                        lis_diag_log_error(err, (char *)__func__, __LINE__, 0);
                        lis_debug_err[0]++;
#endif
                        return(err) ;
                }
    ...
    {
            strbuf32_t kbuf32;
            if ((err = lis_copyin(f, &kbuf32, sb, strbuflen)) < 0)
            {
#ifdef LIS_HAS_PROC_ENTRIES
                    lis_diag_log_error(err, (char *)__func__, __LINE__, 0);
                    lis_debug_err[0]++;
            ...
            kbuf.maxlen = kbuf32.maxlen;
            kbuf.len = kbuf32.len;
            kbuf.buf = (char*) lis_compat_ptr(kbuf32.buf);
    }
...
#ifdef LIS_HAS_PROC_ENTRIES
        lis_diag_log_error(ENXIO, (char *)__func__, __LINE__, 1);
        lis_debug_err[1]++;
#endif
        return(-ENXIO);
    }
...
#ifdef LIS_HAS_PROC_ENTRIES
        lis_diag_log_error(-ERANGE, (char *)__func__, __LINE__, 2);
        lis_debug_err[2]++;
#endif
	return(-ERANGE);
    }
...
#ifdef LIS_HAS_PROC_ENTRIES
        lis_diag_log_error(-ENOMEM, (char *)__func__, __LINE__, 3);
	lis_debug_err[3]++;
#endif
...
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(err, (char *)__func__, __LINE__, 4);
	    lis_debug_err[4]++;
#endif
...
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(err, (char *)__func__, __LINE__, 5);
	    lis_debug_err[5]++;
#endif
...
copyin_msg(struct file *f, stdata_t *hd,
	mblk_t **m, strbuf_t *uctl, strbuf_t *udat, unsigned int ioctl32)
{
    int err;
    mblk_t *aux = NULL;
    ...
    *m = NULL ;					/* initially */
    if ( uctl != NULL )
    {
	if ((err=copyin_msgpart(f,hd,m,(char*)uctl,0, ioctl32))<0)
	{
	    if (*m != NULL)
		lis_freeb(*m);
	    *m = NULL ;
...
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(err, (char *)__func__, __LINE__, 6);
            lis_debug_err[6]++;
#endif
	    return(err);
	}
	...
	*m = NULL ;				/* no ctl msg part */
    ...
    if ( udat != NULL )
    {
	if ((err=copyin_msgpart(f,hd,&aux,(char*)udat,hd->sd_wroff, ioctl32))<0)
	{
	    if (*m != NULL)
		lis_freeb(*m);
...
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(err, (char *)__func__, __LINE__, 7);
            lis_debug_err[7]++;
#endif
		...
		unsigned int ioctl32)
{
    ...
    {				/* ensure writeable */
	if (usb && (err=lis_check_umem(f,VERIFY_WRITE,usb,strbuflen))<0)
	  return(err);
...
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(err, (char *)__func__, __LINE__, 8);
	    lis_debug_err[8]++;
#endif
	...
	if (usb != NULL)
		...
		if (!ioctl32)
			...
			{
			strbuf32_t tmp;
			tmp.maxlen = sb->maxlen;
			tmp.len = sb->len;
			tmp.buf = (unsigned int)(long)sb->buf;
			err = lis_copyout(f,&tmp,usb,sizeof(strbuf32_t));
			}
		}
    }
    else
...
#ifdef LIS_HAS_PROC_ENTRIES
        lis_diag_log_error(err, (char *)__func__, __LINE__, 9);
	lis_debug_err[9]++;
#endif
...
copyout_msg( struct file *f, mblk_t *m, strbuf_t *kctl, strbuf_t *kdta,
	     strbuf_t *uctl, strbuf_t *udta, int doit, unsigned int ioctl32)
{
    int errctl,errdta;

    if ((errctl=copyout_msgpart(f,m,kctl,uctl,1,doit, ioctl32))<0)
    {
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(errctl, (char *)__func__, __LINE__, 10);
            lis_debug_err[10]++;
#endif
    ...
    else if ((errdta=copyout_msgpart(f,m,kdta,udta,0,doit, ioctl32))<0)
    {
#ifdef LIS_HAS_PROC_ENTRIES
            lis_diag_log_error(errdta, (char *)__func__, __LINE__, 11);
            lis_debug_err[11]++;
#endif
...
void	lis_stream_error(stdata_t *shead, int rderr, int wrerr)
{
	...
	SET_SD_FLAG(shead,STRDERR);
        ...
        if(rderr != EIO)
        {
                lis_diag_log_error(rderr, (char *)__func__, __LINE__, 12);
                lis_debug_err[12]++;
        }
#endif
	...
	SET_SD_FLAG(shead,STWRERR);
        ...
        if(wrerr != EIO)
        {
                lis_diag_log_error(wrerr, (char *)__func__, __LINE__, 13);
                lis_debug_err[13]++;
        }
#endif
...
#ifdef LIS_HAS_PROC_ENTRIES
        lis_diag_log_error(err, (char *)__func__, __LINE__, 14);
	lis_debug_err[14]++;
#endif
...
#ifdef LIS_HAS_PROC_ENTRIES  
        if(F_ISSET(shead->sd_flag,STRPRI) && q->q_first == NULL)
	{
#if 0
            lis_diag_log_event(STRPRI_SET_PUSH_HIP, (char *)__func__, __LINE__, 15);
#endif
            lis_debug_err[15]++;	

#if 0
            printk("%s: Trying to push HIP when is set already....ERROR\n",__FUNCTION__);
#endif
	}
#endif	
...
#ifdef LIS_HAS_PROC_ENTRIES
#if 0
                lis_diag_log_event(STRPRI_SET_EMPTY, (char *)__func__, __LINE__, 16);
#endif
                lis_debug_err[16]++;	
#endif
            }
...
#ifdef LIS_HAS_PROC_ENTRIES  
#if 0
                lis_diag_log_event(STRPRI_SET_NOT_EMPTY, (char *)__func__, __LINE__, 17);
#endif
	    	lis_debug_err[17]++;
#endif
	    	CP(q,mp) ;
...
int	_RP lis_strwsrv(queue_t *q)
{
    ...
    {
	/* What on earth ends up on this queue? */
	CP(q,mp) ;
#ifdef POLLFCDEBUG
	printk(
	"lis_strwsrv: putting pm=0x%p from head wsrv!!!\n", mp);
#endif
   ...
   if (hd->sd_sigflags != 0)
   {
	if (couldput && F_ISSET(hd->sd_sigflags,S_OUTPUT))
	{
	    CP(hd,0) ;
					        ...
					        /* write waiting flow ctrl */
   {
    ...
    /* This may lead to the occasional false wake up if putmsg happens to
     * fill up the queue in another thread between the top of this function 
     * and here, but that seems like a pretty small harmless hole 
     * compared to having to do bcanputnext twice with all it's locking... */
    if ( couldput && (POLL_WAITING(hd) || F_ISSET(hd->sd_flag,STRSELPND)) )
    {
	CP(hd,0) ;
#ifdef POLLFCDEBUG
	if (hd->sd_wq->q_next->q_flags & (QFULL|QWASFULL))
		printk(
	"lis_strwsrv: waking poll thread, but QFULL|QWASFULL flags=0x%x\n", 
			hd->sd_wq->q_next->q_flags);
#endif
...
#ifdef DANDEBUG
printk("lis_strdoioctl: f=0x%p hd=0x%p ioc=0x%p\n", f, hd, ioc);
printk("lis_strdoioctl: ic_cmd=0x%x ic_timout=0x%x ic_len=%d ic_dp=0x%p\n", 
	ioc->ic_cmd, ioc->ic_timout, ioc->ic_len, ioc->ic_dp);
#endif
...
#define RTN(v)	do { errv=(v); goto return_point; } while (0)
     ...
     */
    iocb->ioc_id = hd->sd_iocseq =  lis_incr((int*)&lis_iocseq);
  ...
  again:				/* wioc lock is always held when here */
    ...
    case M_COPYIN:
	{
	    ...
	    {
		if (err < 0)
		    cr->cp_rval=(caddr_t)(unsigned long)(-err);
		else
		  ...
		  int this_open, dev_t *ndev, cred_t *creds)
{
    ...
    if (LIS_DEBUG_OPEN || LIS_DEBUG_REFCNTS)
	printk("lis_open_fifo(i@0x%p/%d,f@0x%p/%ld,h@0x%p/%d/%d)#%d"
	       " \"%s\"\n",
	       i, I_COUNT(i),
	       f, F_COUNT(f),
	       head, LIS_SD_REFCNT(head), LIS_SD_OPENCNT(head),
 ...
 */
void check_for_wantenable(stdata_t *hd)
{
    lis_flags_t  psw ;
    unsigned	 flags ;
    queue_t	*q ;
...
lis_stropen( struct inode *i, struct file *f )
{
    ...
    /*********************************************/
    /* Check if LiS has completed initialization */
    /*********************************************/

    if(lis_initialized == 0)
    {
        printk("LiS: %s: WARNING LiS initialization not completed\n", __func__);
        return(-ENXIO);
    }
...
retry_from_start:			/* retry point for open/close races */
    ...
    creds.cr_uid  = (uid_t) EUID(f).val;
    creds.cr_gid  = (gid_t) EGID(f).val;
    creds.cr_ruid = (uid_t) UID(f).val;
    creds.cr_rgid = (gid_t) GID(f).val;
    ...
    if ((err = lis_set_q_sync(head->sd_wq, LIS_QLOCK_NONE)) == 0)
...
#define RTN(e)  do { err=(e); goto return_point; } while (0)
    ...
    if (hd->lis_trace)
    {
        lis_diag_log_trace((char *)__func__, __LINE__, hd->lis_trace_process_name, hd->lis_trace_fd);
    }
...
#define	RTN(val)  do { err = val; goto return_point; } while (0)
	...
	case M_DATA:
	    ...
	    if (LIS_DEBUG_READ)
	      ...
	      "read %d error %d\n",
		   hd->sd_name, hd->sd_rdopt, (int)ulen, count, err) ;
...
#define	RTN(val)  do { err = val; goto return_point; } while (0)
    ...
    if ( (err=copyin_msg(fp,hd,&msg,ctl,dat,ioctl32))<0)	/* allocates msg */
    {
	if (msg)
	    freemsg(msg) ;
	RTN(err);
...
#define RTN(v)	do { err=(v); goto err_return_point; } while (0)
	...
	RTN(-hd->sd_rerror);
            ...
            (err=lis_check_umem(fp,VERIFY_READ,ctl,strbuflen))<0)
       RTN(err);
    else if (doit && dat != NULL &&
            (err=lis_check_umem(fp,VERIFY_READ,dat,strbuflen))<0)
       RTN(err);
    else if (flagsp == NULL)
	RTN(-EFAULT);
...
#ifdef DANDEBUG
printk("getmsg: doit=%d ctl=0x%p dat=0x%p flagsp=0x%p bandp=0x%p\n",
	doit, ctl, dat, flagsp, bandp);
#endif
	...
	if (ctl != NULL)
	{
	    if (!ioctl32)
		{
		if ((err = lis_copyin(fp,&kctl,ctl,strbuflen)) < 0)
			RTN(err) ;
		}
	    else
		{
		if ((err = lis_copyin(fp,&kctl32,ctl,strbuflen)) < 0)
			RTN(err) ;
		kctl.maxlen = kctl32.maxlen;
		kctl.len = kctl32.len;
		kctl.buf = (char*)lis_compat_ptr(kctl32.buf);
#ifdef DANDEBUG
printk("getmsg: kctl.maxlen=%d len=%d buf=0x%p\n", 
	kctl.maxlen, kctl.len, kctl.buf);
#endif
		}
	}
	else
	...
	if (dat != NULL)
	{
	    if (!ioctl32)
		{
		if ((err = lis_copyin(fp,& kdat, dat, strbuflen)) < 0)
			RTN(err) ;
		}
	    else
		{
		if ((err = lis_copyin(fp,&kdat32,dat,strbuflen)) < 0)
			RTN(err) ;
		kdat.maxlen = kdat32.maxlen;
		kdat.len = kdat32.len;
		kdat.buf = (char*)lis_compat_ptr(kdat32.buf);
#ifdef DANDEBUG
printk("getmsg: kdat.maxlen=%d len=%d buf=0x%p\n", 
	kdat.maxlen, kdat.len, kdat.buf);
#endif
		}
	}
	else
...
#ifdef DANDEBUG
printk("getmsg: flags=0x%x band=%d\n", flags, band);
#endif
    ...
    {
	/* This is for I_PEEK, we already converted the structs from 32 to 
	 * 64 bit, so this is ok.. */
	kctl  = *ctl ;
	kdat  = *dat ;
	flags = *flagsp ;
	band  = *bandp ;
	...
	rtn=copyout_msg(fp,NULL,&kctl,&kdat,
		    (strbuf_t*)ctl,(strbuf_t*)dat, doit, ioctl32) ;
    ...
    {
	printk("lis_strgetpmsg: q=0x%p, mp=0x%p, mp->b_rptr=0x%p\n",
				rdq, mp, mp->b_rptr);
    }
	...
	if ( mp == NULL )
	{
	    printk("strgetpmsg: empty queue: addr of queue 0x%p\n",
		    rdq);
	    err = -EINVAL;
	    goto return_point;
	}
    ...
    if ((rtn=copyout_msg(fp,mp,&kctl,&kdat,
			 (strbuf_t*)ctl,(strbuf_t*)dat,doit, ioctl32)) < 0)
	lis_freemsg(mp);
    else 
...
/* This wrapper is necessary because we now have an extra parameter to
 * lis_strioctl that we have to set. */
int
lis_strioctl_wrapper(struct inode *i, struct file *f, unsigned int cmd, unsigned long arg )
{
    return(lis_strioctl(i, f, cmd, arg, 0));
}
/* strioctl_unlocked - handle IOCTLs at the stream head
 * This is the new ioctl interface (starting in 2.6.11) 
 * which does not grab the BKL (Big Kernel Lock).  
 * Of course, since "different _is_ better", they could not leave 
 * the calling parameters alone so we have to have this wrapper function.
 */
long
lis_strioctl_unlocked(struct file *f, unsigned int cmd, unsigned long arg )
{
    struct inode *i;
...
long
lis_strioctl_compat(struct file *f, unsigned int cmd, unsigned long arg )
{
    struct inode *i;
...
#ifdef DANDEBUG
    printk("lis_strioctl_compat: i=0x%p cmd=0x%x arg=0x%lx compat=0x%lx\n",
	i, cmd, arg, (unsigned long) lis_compat_ptr(arg));
#endif

    return(lis_strioctl(i, f, cmd, (unsigned long) lis_compat_ptr(arg), 1));
}
...
lis_strioctl( struct inode *i, struct file *f, unsigned int cmd,
	      unsigned long arg, unsigned int ioctl32)
{
    stdata_t *hd;
    ...
    unsigned long  time_cell = 0 ;
    pid_t pid;
    extern int lis_work_incr;
    extern int lis_scheduler_policy;

#define RTN(v)	do { err=(v); goto return_point; } while (0)

    if (cmd == I_LIS_GETPMSG)
    { 
	getpmsg_args_t	args ;
	...
	if (!ioctl32)
		...
		if ((err = lis_check_umem(f, VERIFY_READ, (char*)arg, 
						sizeof(args))) || 
		    (err = lis_copyin(f, &args, (char*)arg, sizeof(args))))
                {
			printk(
	"lis_strioctl: getpmsg: failed to copyin arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args));
			return(err);
                }
        }
	else
        {
		getpmsg32_args_t	args32 ;
		if ( (err = lis_check_umem(f, VERIFY_READ, (char*)arg, 
					 sizeof(args32))) )
                {
                        printk(
	"lis_strioctl: getpmsg32: failed to check umem arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args32));
			return(err);
			}
		if ((err = lis_copyin(f, &args32, (char*)arg, sizeof(args32))))
                {
			printk(
	"lis_strioctl: getpmsg32: failed to copyin arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args32));
			return(err);
                }
		args.fd = args32.fd;
		args.ctl = (struct strbuf*) lis_compat_ptr(args32.ctl);
		args.dat = (struct strbuf*) lis_compat_ptr(args32.dat);
		args.bandp = (int*) lis_compat_ptr(args32.bandp);
		args.flagsp = (int*) lis_compat_ptr(args32.flagsp);
#ifdef DANDEBUG
		printk(
  "lis_strioctl: getmsg: args.ctl=0x%p dat=0x%p bandp=0x%p flagsp=0x%p\n",
			args.ctl, args.dat, args.bandp, args.flagsp);
#endif
        }
#ifdef DANDEBUG
	printk(
	"lis_strioctl: about to call strgetpmsg: args.ctl=0x%p dat=0x%p "
	"bandp=0x%p flagsp=0x%p\n",
		args.ctl, args.dat, args.bandp, args.flagsp);
#endif
	err = lis_strgetpmsg(i, f, args.ctl, args.dat,
				 args.bandp, args.flagsp, 1, ioctl32);
	...
	putpmsg_args_t	args ;
		...
		if ((err = lis_check_umem(f, VERIFY_READ, (char*)arg, 
		    ...
		    (err = lis_copyin(f, &args, (char*)arg, sizeof(args))))
                {
			printk(
	"lis_strioctl: putpmsg: failed to copyin arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args));
			return(err);
                }
        }
        ...
        {
		putpmsg32_args_t	args32 ;
		if ( (err = lis_check_umem(f, VERIFY_READ, (char*)arg, 
					 sizeof(args32))) )
                {
			printk(
	"lis_strioctl: putpmsg32: failed to check umem arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args32));
			return(err);
                }
		if ( (err = lis_copyin(f, &args32, (char*)arg, 
					sizeof(args32))) )
                {
			printk(
	"lis_strioctl: putpmsg32: failed to copyin arg=0x%p size=%d\n",
				(char*)arg, (int) sizeof(args32));
			return(err);
                }
		args.fd = args32.fd;
		args.ctl = (struct strbuf*) lis_compat_ptr(args32.ctl);
		args.dat = (struct strbuf*) lis_compat_ptr(args32.dat);
		args.band = args32.band;
		args.flags = args32.flags;
        }

	err = lis_strputpmsg(i, f, args.ctl, args.dat, 
			     args.band, args.flags, ioctl32);
	...
	printk("lis_strioctl(i@0x%p/%d,f@0x%p/%ld,cmd=0x%x,arg=0x%lx)"
	       " << i_rdev=0x%x\n",
	       i, (i?I_COUNT(i):0),
	       f, (f?F_COUNT(f):0),
	...
	if (!ioctl32)
		...
		err = lis_copyout(f,&strpeek32,(char*)arg,
				  sizeof(strpeek32_t));
		}
	...
	if (!ioctl32)
		strioctllen = sizeof(strioctl_t);
	else
		strioctllen = sizeof(strioctl32_t);
	...
	if ((err=lis_check_umem(f,VERIFY_READ,
	    ...
	    (err=lis_check_umem(f,VERIFY_WRITE,
			    (char*)arg,sizeof(strioctl_t)))<0)
		{
		printk("I_STR: err=%d arg=0x%lx\n", err, arg);
		RTN(err);
		}
	if (!ioctl32)
		{
		if ( (err = lis_copyin(f,&ioc,(char*)arg,
					sizeof(strioctl_t))) < 0 )
			{
			printk("I_STR: copyin ioc=0x%p arg=0x%lx size=%u\n",
				&ioc, arg, 
				(unsigned int)sizeof(strioctl_t));
			RTN(err);
			}
		}
	else
		{
		strioctl32_t ioc32;
		if ( (err = lis_copyin(f,&ioc32,(char*)arg,
					sizeof(strioctl32_t))) < 0 )
			{
			printk("I_STR: copyin ioc=0x%p arg=0x%lx size=%u\n",
				&ioc, arg, 
				(unsigned int)sizeof(strioctl_t));
		...
		ioc.ic_cmd = ioc32.ic_cmd;
		ioc.ic_timout = ioc32.ic_timout;
		ioc.ic_len = ioc32.ic_len;
		ioc.ic_dp = (char*) lis_compat_ptr(ioc32.ic_dp);
		}
	CLOCKADD() ;
	...
	if (!ioctl32)
		...
		{
		strioctl32_t ioc32;
		ioc32.ic_cmd = ioc.ic_cmd;
		ioc32.ic_timout = ioc.ic_timout;
		ioc32.ic_len = ioc.ic_len;
		ioc32.ic_dp = (unsigned int)(long)(ioc.ic_dp);
		res = lis_copyout(f,&ioc32,(char*)arg,sizeof(strioctl32_t));
		}

	if ( LIS_DEBUG_IOCTL )
		printk("lis_strioctl(...,I_STR,...) "
		" \"%s\" >> ic_cmd=0x%x ic_timout=%d ic_len=%d rslt=%d\n",
			hd->sd_name, ioc.ic_cmd, ioc.ic_timout, 
			ioc.ic_len, res) ;
	...
	if ((err=lis_check_umem(f,VERIFY_READ,(char*)arg,FMNAMESZ+1))<0)
		...
		/* I can't believe that this is correct... We only
		 * set this if we push a module?  WTF? */
	...
	if (!ioctl32)
		strfdinsertlen = sizeof(strfdinsert_t);
	else
		strfdinsertlen = sizeof(strfdinsert32_t);
	...
	if (!ioctl32)
		{
		if ((err = lis_copyin (f, &strfdinsert, (char *) arg,
					   strfdinsertlen)) < 0)
		    RTN(err);
		}
	else
		{
		strfdinsert32_t strfdinsert32;
		if ((err = lis_copyin(f, &strfdinsert32, (char *) arg,
				     strfdinsertlen)) < 0)
		    RTN(err);
		strfdinsert.ctlbuf.maxlen = strfdinsert32.ctlbuf.maxlen;
		strfdinsert.ctlbuf.len = strfdinsert32.ctlbuf.len;
		strfdinsert.ctlbuf.buf = (char*) 
				lis_compat_ptr(strfdinsert32.ctlbuf.buf);
		strfdinsert.databuf.maxlen = strfdinsert32.databuf.maxlen;
		strfdinsert.databuf.len = strfdinsert32.databuf.len;
		strfdinsert.databuf.buf = (char*) 
				lis_compat_ptr(strfdinsert32.databuf.buf);
		strfdinsert.flags = strfdinsert32.flags;
		strfdinsert.fildes = strfdinsert32.fildes;
		strfdinsert.offset = strfdinsert32.offset;
		}
	...
	if ((err = copyin_msg (f, hd, &msg, ctl, dat, ioctl32)) < 0)
	{
	  if (msg)
	      freemsg(msg) ;
	  RTN(err);
	...
	*((queue_t **) (msg->b_rptr + strfdinsert.offset)) = LIS_RD (q);
	...
	if ((err = lis_check_umem( f, VERIFY_WRITE, (char*)arg,
	    ...
	    /* ZZZ ugh this is surely broken for 32 bit user space over 
	     * 64 bit kernels. */
	...
	if (!arg)		/* want number of modules */
	{
	    ...
	    int strlistlen;
	    if (!ioctl32)
		strlistlen = sizeof(str_list_t);
	    else
		strlistlen = sizeof(str_list32_t);
	    if (   (err=lis_check_umem(f,VERIFY_READ,(char*)arg,
						strlistlen))<0
	    ...
	    if (!ioctl32)
		{
		if ((err=lis_copyin(f,&list,(char*)arg,
				    sizeof(str_list_t)))<0)
			RTN(err);
		}
	    else
		{
		str_list32_t list32;
		if ((err=lis_copyin(f,&list32,(char*)arg,
				sizeof(str_list32_t)))<0)
			RTN(err);
		list.sl_nmods = list32.sl_nmods;
		list.sl_modlist = (str_mlist_t*)
				lis_compat_ptr(list32.sl_modlist);
		}
	    if (list.sl_nmods<=0)
		RTN(-EINVAL);
		...
		{
		if ((err = lis_copyout(f, &nmods,
			&((str_list32_t *)arg)->sl_nmods, sizeof(int))) < 0)
		RTN(err) ;
		}
	...
	stats->work_threshold = lis_work_incr;
	stats->sched_policy = lis_scheduler_policy;
	...
	for (i = 0; i < LIS_NR_CPUS; i++)
		{
		stats->runq_cnts[i] = lis_runq_cnts[i];
		stats->runq_srv_rpt_cnts[i] = lis_runq_srv_rpt_cnts[i];
		stats->queuerun_cnts[i] = lis_queuerun_cnts[i];
		stats->active_flags[i] = K_ATOMIC_READ(&lis_runq_active_flags[i]);
		stats->runq_pids[i] = lis_runq_pids[i];
		stats->runq_wakeups[i] = lis_runq_wakeups[i];
		stats->setqsched_cnts[i] = lis_setqsched_cnts[i];
		stats->setqsched_isr_cnts[i] = lis_setqsched_isr_cnts[i];
		stats->setqsched_usr_cnts[i] = lis_setqsched_usr_cnts[i];
		stats->setqsched_defer_cnts[i] = lis_setqsched_defer_cnts[i];
		stats->setqsched_poke_cnts[i] = lis_setqsched_poke_cnts[i];
		}
...
#if 0
	unsigned long tmp;
	unsigned long stats[STRMAXSTAT][4];
#else
	unsigned int tmp;
	unsigned int stats[STRMAXSTAT][4];
#endif
        unsigned int i, j;
	...
	tmp = K_ATOMIC_READ(&lis_spin_lock_count) ;
	K_ATOMIC_SET(&lis_strstats[LOCKCNTS][TOTAL], tmp);
	tmp = K_ATOMIC_READ(&lis_spin_lock_contention_count);
	K_ATOMIC_SET(&lis_strstats[LOCKCNTS][FAILURES], tmp);

	for (i = 0; i < STRMAXSTAT; i++)
		for (j = 0; j < 4; j++)
			{
			stats[i][j] = K_ATOMIC_READ(&lis_strstats[i][j]);
			}
    ...
    case I_LIS_PRNTBUF:			/* LiS only - print buffers */
        lis_print_buf((int) arg) ;
	break;
    ...
    case I_LIS_SET_WORK_INCR:		/* LiS only - set number of queues
					 * waiting before we start a new 
					 * qrunner */
	{
	if ((err = lis_check_umem(f, VERIFY_READ, (char*)arg,
					sizeof(lis_work_incr))) < 0 || 
	    (err = lis_copyin(f, &lis_work_incr, (char*)arg,
				sizeof(lis_work_incr))) < 0)
		RTN(err);
	...
	printk(
"LiS: Set maximum queue depth for new queue runners to %d\n", 
		lis_work_incr);
	}
	break;

    /* LiS only - set CPUs to run queue runner threads */
    case I_LIS_SET_CPUS:		
	{
	...
	unsigned int ncpus;
	char	name[16] ;
	extern struct completion lis_runq_kill_sems[LIS_NR_CPUS];
	extern char	lis_version[] ;
	extern int	lis_thread_runqueues(void *p);
	extern void	lis_kill_qsched(void);

        if ((err = lis_check_umem(f ,VERIFY_READ, (char*)arg, 
				  sizeof(unsigned int))) < 0)
		RTN(err);
	if ((err = lis_copyin(f, &cpus_allowed, (char*)arg, sizeof(int))) < 0)
		RTN(err);
	if (cpus_allowed == 0)
		RTN(-EINVAL) ;

	ncpus = NUM_CPUS ;
	if (cpus_allowed >= (1<<(ncpus+1)))
		RTN(-EINVAL);

	lis_kill_qsched();
	if (K_ATOMIC_READ(&lis_runq_cnt))
		{
		printk(
 "LIS_SET_CPUS: failed to kill all queue runners! %ld still running\n", 
			lis_runq_cnt);
		}
	for (cpu = 0; cpu < ncpus; cpu++)
		{
		if (!(cpus_allowed & (1<<cpu)))
			continue;
		
		if (lis_runq_pids[cpu] > 0)		/* already running */
			{
			printk(
 "LIS_SET_CPUS: failed to start thread on CPU %d! It's already running as pid %d\n", 
				cpu, lis_runq_pids[cpu]);
			continue ;
			}

		/* initialize semaphore */
		init_completion(&lis_runq_kill_sems[cpu]) ;
		...
		if (lis_runq_pids[cpu] < 0)		/* failed to fork */
			{
			printk(
		"lis_start_qsched: %s: lis_thread_start error %d\n",
    ...
    /* LiS only - set queue runner thread scheduling policy */
    case I_LIS_SET_POLICY:
    {
	int type;

        if ((err = lis_check_umem(f ,VERIFY_READ, (char*)arg, sizeof(int))) < 0)
		RTN(err);
	if ((err = lis_copyin(f, &type, (char*)arg, sizeof(int))) < 0)
		RTN(err);

	if (type < LIS_SCHEDULER_RRP || type > LIS_SCHEDULER_LAST)
		RTN(-EINVAL) ;
	lis_scheduler_policy = type;

        if(type == LIS_SCHEDULER_RRP)
        {
                printk("LiS: Scheduling Policy set to Round Robin\n");
        }
        else
        {
                printk("LiS: Scheduling Policy set to Last Processor First\n");

        }
    }
    break;

    case I_LIS_TRACE:		/* LiS only - enable system call tracing for this FD */
    {
        /* argument structure for the I_LIS_TRACE ioctl */
        lis_trace_args_t lis_trace;
        
        if ((err = lis_check_umem(f ,VERIFY_READ, (char*)arg, sizeof(lis_trace_args_t))) < 0)
        {
            RTN(err);
        }

        if ((err = lis_copyin(f, &lis_trace, (char*)arg, sizeof(lis_trace_args_t))) < 0)
        {
            RTN(err);
        }

        hd->lis_trace_fd = lis_trace.lis_trace_fd;
        
        strncpy (hd->lis_trace_process_name,lis_trace.lis_trace_process_name,
                 sizeof(lis_trace.lis_trace_process_name));
            
        hd->lis_trace = 1;

	printk("LiS: System Call Trace Enabled for %s, FD %d\n", 
               hd->lis_trace_process_name, hd->lis_trace_fd);
    }
    break;
...
#define	RTN(msk)	do { revents = msk; goto rtn_point; } while (0)
    ...
    if ( F_ISSET(hd->sd_flag,(STRDERR|STWRERR|STPLEX)) )
	{
	    if (F_ISSET(hd->sd_flag,STPLEX))
	    {
		if (LIS_DEBUG_POLL)
		  printk(
		"strpoll: stream %s: stream has been I_LINKed\n", hd->sd_name);
		RTN(POLLNVAL) ;
	    }
	/* Sadly we don't get the user's poll flags, so we can't tell
	 * the difference if they polled for RDERR or WRERR, we return POLLERR
	 * in either case... */
	RTN(POLLERR) ;
	}
    ...
    /* Find the next queue downstream which has a service routine */
    /* ZZZ should be locked, but the way that it's done in bcanput is not 
     * correct either since it doens't lock each queue as it goes down
     * the stream.  We really need an explict pointer to the next downstream
     * queue with a service routine instead of having to do this linear search
     * for every bcanput.. */
    nq = hd->sd_wq->q_next;
    while (   nq->q_next != NULL
	   && (nq->q_qinfo == NULL || nq->q_qinfo->qi_srvp == NULL)
          )
	    nq = nq->q_next;

    /* This does not have to be locked since it's a single read cycle */
    q_flag = nq->q_flag;

    if ( !(q_flag & (QFULL|QWASFULL)) )
	revents |= (POLLWRNORM | POLLOUT) ;	/* can write band 0 data */
    /* We don't care q_flag was not locked, because there are only two race 
     * cases:
     * q_flag has Q_FULL set and it's not set now:  In this case we would still
     * want QWANTW set so that the stream head gets backenabled and wakes up
     * poll again when QWASFULL is cleared.
     * q_flag does not have Q_FULL set and it's set now:  In this case there
     * must be another thread doing a putmsg() at the same time and it will set
     * QWANTW.
     */
    else if (q_flag & QFULL) 
	{
	LIS_QISRLOCK (nq, &psw);
	nq->q_flag |= QWANTW;
	LIS_QISRUNLOCK (nq, &psw);
	}
    for (qp = nq->q_bandp; qp != NULL; qp = qp->qb_next)
	{
...
void	_RP lis_wakeup_close(caddr_t arg)
{
	...
	hd->sd_close_timer = 0 ;
	complete(&hd->sd_closing) ;			/* semaphore wakeup */
    }
    else
 ...
 */
void mark_closing(queue_t *sd_wq)
{
    queue_t	*sd_rq ;
    queue_t	*q ;
 ...
 */
static void close_action(stdata_t *head, struct file *fp)
{
    int		rslt ;
    stdata_t   *hd_peer ;
    lis_flags_t	psw ;
	...
	&& lis_qcountstrm(head->sd_wq) != 0	/* messages queued */
        && fp 
	&& !should_notblock_rd(head, fp)	/* not NDELAY or NONBLOCK */
       )
    {
	/* set a timer */
	CP(head,0) ;
	...
	lis_runqueues() ;		
	/* sleep for awhile */
	wait_for_completion(&head->sd_closing) ;
	K_ATOMIC_INC(&lis_in_syscall) ;	/* processing a system call */
...
lis_doclose( struct inode *i, struct file *f, stdata_t *head, cred_t *creds )
{
    ...
    {
	printk("lis_doclose(i@0x%p/%d,f@0x%p/%ld,h@0x%p/%d/%d.%d,...)#%ld/%d\n"
	       "    << \"%s\" "
#if defined(LINUX) && defined(KERNEL_2_1)
	       "d@0x%p/%d "
#endif
	...
	close_action(head, f) ;			/* perform close functions */
    }
...
#define RTNX	return(0)
    ...
    if ((LIS_DEBUG_CLOSE || LIS_DEBUG_ADDRS) || LIS_DEBUG_REFCNTS)
	printk("lis_strclose(i@0x%p/%d,f@0x%p/%ld)#%ld i_rdev=(%d,%d)\n"
	       "    << "
#if defined(LINUX) && defined(KERNEL_2_1)
	       "d@0x%p/%d "
#endif
