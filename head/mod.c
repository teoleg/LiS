...
volatile int lis_fmodcnt; /* (largest module id used) + 1 */
...
lis_loadmod(const char *name)
{
	...
	if ((err = lis_down(&lis_mod_reg)) < 0)
        ...
        /* try to find the module (already loaded or loading) */
	id = lis_findmod(name);
...
#else
            printk("Bad module state for %s (LOADING), id %d \n", name, id) ;
#endif
...
#if 0
/* This whole hunk of code only exists to call ldl_init() which is called
 * anyways when the module is loaded.  I really have no idea why this is needed.
 * Since inter_module_get_request and inter_module_put are being removed in
 * 2.6.16 kernels unless the kernel is explicitly set to enable it, I'm
 * just going to remove all of this.  These LiS modules really need to be
 * reworked to not be so goofy, but since we don't use any of them that's
 * a REALLY low priority project.
 * dg@adax.com Jan 10, 2007
 */
