...
lis_kmem_cache_t *lis_locks_cachep;
#endif
...
// This should be configurable based on the number of CPU cores that
// we are running on, in the case of the MIPS Octeon, we have 16 cores
#define	LOCK_CONTENTION_SIZE	16	/* # locks to keep track of */
...
#else
/* generic semaphores were introduced in 2.6.25-et1 
...
#define	SEM_ENTRY(sem,typ,fil,lin,flgs)					\
	...
	p->cpu  = get_cpu();					\
	put_cpu(); \
	p->state = ((struct semaphore *) &sem->sem_mem)->count; \
    }
#define SEM_EXIT(sem,typ,fil,lin,flgs)					\
    if ( LIS_DEBUG_SPL_TRACE )						\
    {									\
...
void lis_init_locks(void)
{
    ...
    lis_locks_cachep = lis_kmem_cache_create("lis_locks_cache", size, 0,
					 SLAB_HWCACHE_ALIGN);
}
