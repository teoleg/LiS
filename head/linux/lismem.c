...
/* FIXME: All this is pretty unnecessary now days.  kmalloc itself uses
 * the slab allocator, so there is really no reason to have a separate
 * LiS slab cache for lis_kmalloc, just call kmalloc()... */
void lis_mem_init(void)
{
	...
	{
	    p->cache_struct = lis_kmem_cache_create(p->name, p->size, 0,
