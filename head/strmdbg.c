...
/* ### No KMEM-CACHE Work */
#if 0
	memset(p->file_name, 0, sizeof(p->file_name)) ;
#endif
...
#if defined(LINUX)			/* Linux kernel version */
    if (unlikely(abytes > MAX_KMALLOC_SIZE))
...
#if defined(LINUX)				/* Linux kernel version */
    if (unlikely(p->size > MAX_KMALLOC_SIZE))
...
/* ### No KMEM-CACHE Work */
        ...
        case BLK_MB_HOLD:
        {
                printk("lis: %ld bytes (at %p) of type ALLOCHDR (0xaa1) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
           
                lis_print_msg((mblk_t *)(pp+1), p->file_name,PRINT_DATA_ENTIRE);

                break;
        }
        case BLK_MB_FREED:
        {
                printk("lis: %ld bytes (at %p) of type FREED (0xaa2) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_TIMER:
        {
                printk("lis: %ld bytes (at %p) of type TIMER (0xaa3) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_HEAD:
        {
                printk("lis: %ld bytes (at %p) of type HEAD (0xaa4) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_QUEUE:
        {
                printk("lis: %ld bytes (at %p) of type QUEUE (0xaa5) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_MB_PUTNEXT:
        {
                printk("lis: %ld bytes (at %p) of type PUTNEXT (0xaa6) to %s\n",
                       p->size, pp+1, p->file_name);
        ...
        case BLK_MALLOC:
        {
                printk("lis: %ld bytes (at %p) of type MALLOC (0xaa7) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_FREE:
        {
                printk("lis: %ld bytes (at %p) of type FREE (0xaa8) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_MB_PUTQ:
        {
                printk("lis: %ld bytes (at %p) of type PUTQ (0xaa9) to %s\n",
                       p->size, pp+1, p->file_name);
        ...
        case BLK_MB_PUTBQ:
        {
                printk("lis: %ld bytes (at %p) of type PUTBQ (0xaaa) to %s\n",
                       p->size, pp+1, p->file_name);
        ...
        case BLK_MB_GETQ:
        {
                printk("lis: %ld bytes (at %p) of type GETQ (0xaab) from %s\n",
                       p->size, pp+1, p->file_name);
        ...
        case BLK_MB_HDR:
        {
                printk("lis: %ld bytes (at %p) of type HDR (0xaac) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_MB_DB:
        {
                printk("lis: %ld bytes (at %p) of type DB (0xaad) leaked from file %s, line %d\n",
                       p->size, pp+1, p->file_name, p->line_nr);
        ...
        default:
        {
                printk("lis: %ld bytes (at %p) of (unknown) type 0x%x leaked from file %s, line %d\n",
				      p->size, pp+1, p->blk_type, p->file_name, p->line_nr);
...
*									*
* It will also be called if USE_KMEM_CACHE is not defined.              *
*									*
************************************************************************/
/* ### No KMEM-CACHE Work */
#if 0
...
#else
void	_RP lis_mark_mem_fcn(void *ptr, const char *file_name, int line_nr, int type)
#endif
{
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
    mem_link_t		*p ;
...
#else
		printk("mblk: \"%s\", size=%ld, type=0x%x\n", p->file_name, p->size, p->blk_type);
#endif
...
/* ### No KMEM-CACHE Work */
	    ...
	    printk("%s #%d, size=%ld, type=0x%x\n", p->file_name, p->line_nr, p->size, p->blk_type);
#endif
	    break ;
	}
    }
...
*************************************************************************
*									*
* Walk the memory link list and print out each buffer element.		*
*									*
************************************************************************/
void	_RP lis_print_buf(int num_buf)
{
    mem_link_t		*p ;
    mem_link_space_t	*pp ;
    lis_flags_t  	 psw;
    ...
    if(!(num_buf))
    {
        ...
        printk("lis: Printing ALL buffers\n");
    }
    else
    {
        printk("lis: Printing up to %d buffers\n", num_buf);
    }
	...
	pp = (mem_link_space_t *) p ;
        ...
        case BLK_MB_HOLD:
        {
                printk("lis[%d]: %ld bytes (at %p) of type ALLOCHDR (HOLD) (0xaa1) from file %s, line %d\n",
                       i, p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_MB_PUTNEXT:
        {
                printk("lis[%d]: %ld bytes (at %p) of type PUTNEXT (0xaa6) to %s\n",
                       i, p->size, pp+1, p->file_name);
        ...
        case BLK_MB_PUTQ:
        {
                printk("lis[%d]: %ld bytes (at %p) of type PUTQ (0xaa9) to %s\n",
                       i, p->size, pp+1, p->file_name);
        ...
        case BLK_MB_PUTBQ:
        {
                printk("lis[%d]: %ld bytes (at %p) of type PUTBQ (0xaaa) to %s\n",
                       i, p->size, pp+1, p->file_name);
        ...
        case BLK_MB_GETQ:
        {
                printk("lis[%d]: %ld bytes (at %p) of type GETQ (0xaab) from %s\n",
                       i, p->size, pp+1, p->file_name);
        ...
        case BLK_MB_HDR:
        {
                printk("lis[%d]: %ld bytes (at %p) of type HDR (0xaac) from file %s, line %d\n",
                       i, p->size, pp+1, p->file_name, p->line_nr);
        ...
        case BLK_MB_DB:
        {
                printk("lis[%d]: %ld bytes (at %p) of type DB (0xaad) from file %s, line %d\n",
                       i, p->size, pp+1, p->file_name, p->line_nr);
...
#else
    printk("lis: Printing buffers is only available when not using KMEM Cache\n");
#endif
