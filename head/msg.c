 ...
 * RCS Id          : $Id: msg.c,v 9.4 2012/09/28 18:13:01 hecht Exp $
...
#endif
{
	...
	if (lis_max_msg_mem == 0 || K_ATOMIC_READ(&lis_strcount) < lis_max_msg_mem)
        ...
        {
            /* signify header */
	    lis_mark_mem(rtn, file_name, line_nr, BLK_MB_HDR);
        }
#endif
...
/* ### No KMEM-CACHE Work */
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
	    lis_mark_mem(rtn, file_name, line_nr, BLK_MB_HOLD);    /* under new ownership*/
#else
...
freehdr(mblk_t *bp)
{
     ...
     * mblk then leave it alone for post mortem debugging purposes.
...
/* ### No KMEM-CACHE Work */
#if defined(CONFIG_DEV) || !defined(USE_KMEM_CACHE)
    lis_mark_mem(bp, "free", MEM_MSG, BLK_MB_FREED);     /* under new ownership */
#else
...
#endif
{
    ...
    /* signify data block */
    lis_mark_mem(rtn, file_name, line_nr, BLK_MB_DB);
...
lis_freedb(mblk_t *bp, int free_hdr)
{
    	...
    	if (dp)
	{
		...
		/* This is a double free.. Go track it down! */
		dump_stack();
		return;
	    }
	     ...
	     */
            ...
            /* Insure that the ref count is locked for the decrement 
	     * this is to prevent a race where Thread A enters, grabs the lock
	     * and decrements dp->db_ref but does not check dp->db_ref == 0
	     * below, Thread B then enters, sees dp->db_ref == 1, does not grab
	     * the lock then does the decrement.  Before, both threads would 
	     * then test db_ref below and find it to be zero causing both
	     * threads to free the same buffer.  Locking the decrement below
	     * forces Thread B to wait until Thread A is out of this rtn before
	     * doing the comparison below.
	     */
            if (locked)
            {
                    dp->db_ref--;
