 ...
 * RCS Id          : $Id: head.h,v 9.4 2012/05/07 17:57:29 hecht Exp $
...
#error "Either PORTABLE_POLL or LINUX_POLL must be defined"
	...
	struct completion  sd_close_wt; /* Waiting for close to complete*/
	struct completion  sd_wiocing;  /* wait for iocl response */
        struct completion  sd_wread;    /* wait for msg to arrive */
        struct completion  sd_wwrite;   /* wait to room for write */
	struct completion  sd_closing;	/* waiting to close */
	...
	cred_t		   sd_creds ;	/* portable form of creds */
        ...
        int                lis_trace;      /* LiS system call trace enabled (1)/disabled (0)*/ 
...
extern int lis_strioctl_wrapper(struct inode *, struct file *, 
			unsigned int, unsigned long );
extern long lis_strioctl_unlocked(struct file *, unsigned int, unsigned long);
extern long lis_strioctl_compat(struct file *, unsigned int, unsigned long);
...
extern int lis_strgetpmsg(struct inode *, struct file *,
			void *, void *, int *,int *,int, unsigned int);
...
extern void check_for_wantenable(stdata_t *hd);
