 ...
 * RCS Id          : $Id: strconfig.h,v 9.2 2010/03/31 15:19:53 hecht Exp $
...
/* Queue runner thread scheduling policies.  Can be configured by user
 * at run time with the streams -P command */
#define LIS_SCHEDULER_RRP 0	/* Round robin processor.  Start queue runner
				 * threads starting from processor where we
				 * left off last time. */
#define LIS_SCHEDULER_LAST 1	/* Last processor.  Always start from the 
				 * highest numbered processor and work 
				 * backwards until we have enough queue 
				 * runners. */
/*  -------------------------------------------------------------------  */
