 ...
 * MA 02139, USA.
 * 
 * This file is to be included when stropts.h is included with 
 * LIS_USE_IOCTL32 defined into 32 bit apps running on 64 bit kernels 
 * older than 2.6.11 which do not have compat_ioctl file_ops support.  
 *
 * This file redefines the normal streams ioctl values to not collide
 * with cdrom ioctls so that we can register 32 bit to 64 bit conversion
 * functions with register_ioctl32_conversion().
 */

#ifndef _SYS_STROPTS_IOCTL32_H
#define _SYS_STROPTS_IOCTL32_H
#define	_LIS_SYS_STROPTS_IOCTL32_H	/* so you can tell which stropts.h you got */
...
typedef struct strbuf32 {
    int     maxlen;                 /* no. of bytes in buffer */
    int     len;                    /* no. of bytes returned */
    unsigned int buf;                   /* pointer to data */
} strbuf32_t;
...
#define __SID           ('S'<<24 | 'T'<<16 | 'R'<<8)	/* for GNU libc compatibility */
#define STR             ('S'<<24 | 'T'<<16 | 'R'<<8)
 ...
 */
#define I_LIS_GETPMSG 		(__SID |126)	/* getpmsg(), 
						 * implemented as an ioctl */
#define I_LIS_PUTPMSG 		(__SID |127)	/* putpmsg(), 
						 * implemented as an ioctl */

#define I_LIS_TRACE             (__SID |128)    /* enable LiS system call trace */

#define	I_LIS_PRNTBUF		(__SID |235)
#define I_LIS_SET_POLICY	(__SID |236)
#define I_LIS_SET_CPUS		(__SID |237)	
#define I_LIS_SET_WORK_INCR	(__SID |238)	
...
typedef struct str_list32 {
    int               sl_nmods;
    unsigned int      sl_modlist;
} str_list32_t;
...
typedef struct
{
    ...
    unsigned long	num_qrunners ;
    unsigned long	queues_running ;
    unsigned long	runq_req_cnt ;
    unsigned long	runq_cnts[LIS_NR_CPUS] ;
    unsigned long	runq_srv_rpt_cnts[LIS_NR_CPUS] ;
    unsigned long	queuerun_cnts[LIS_NR_CPUS] ;
    unsigned long	active_flags[LIS_NR_CPUS] ;
    unsigned long	runq_pids[LIS_NR_CPUS] ;
    ...
    unsigned long	setqsched_usr_cnts[LIS_NR_CPUS] ;
    unsigned long	setqsched_defer_cnts[LIS_NR_CPUS] ;
    unsigned long	setqsched_poke_cnts[LIS_NR_CPUS] ;
} lis_qrun_stats_t ;
...
/*  *******************************************************************  */
/*
 * Structures for I_LIS_TRACE - enable LiS System Call Trace
 *
 */

typedef struct lis_trace_args
{
    int  lis_trace_fd;   /* FD for the trace */
    char lis_trace_process_name[16]; /* Process that registered the trace */
