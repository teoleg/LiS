...
#define LIS_HAS_PCI_SET_CONSISTENT_DMA_MASK 1
int	lis_pci_set_consistent_dma_mask(lis_pci_dev_t *dev, u64 mask)_RP __attribute__((weak));
	
#define LIS_HAS_PCI_RESOURCE_FCNS 1
unsigned long long lis_pci_resource_start(lis_pci_dev_t *dev, unsigned int bar)_RP __attribute__((weak));
unsigned long long lis_pci_resource_end(lis_pci_dev_t *dev, unsigned int bar)_RP __attribute__((weak));
unsigned long long lis_pci_resource_len(lis_pci_dev_t *dev, unsigned int bar)_RP __attribute__((weak));
unsigned long lis_pci_resource_flags(lis_pci_dev_t *dev, unsigned int bar)_RP __attribute__((weak));
...
#define LIS_HAS_PCI_REGION_FCNS 1
#define LIS_IORESOURCE_IO 0x100
#define LIS_IORESOURCE_MEM 0x200
#define LIS_IORESOURCE_PREFETCH 0x1000
int lis_pci_select_bars(lis_pci_dev_t *dev, unsigned long flags) _RP __attribute__((weak));
int lis_pci_request_selected_regions(lis_pci_dev_t *dev, int bars, 
				     char *res_name) _RP __attribute__((weak));
void lis_pci_release_selected_regions(lis_pci_dev_t *dev, int bars) _RP __attribute__((weak));

#define LIS_HAS_PCI_REGISTER_DRIVER 1
/* New style PCI driver API stuff */
struct lis_pci_device_id {
	unsigned int vendor;
	unsigned int device;
...
struct lis_pci_driver {
	struct lis_pci_driver *next;
	char *name;
	struct lis_pci_device_id *id_table;
	void *linux_pci_driver;
	void *linux_pci_id_table;
	int _RP (*lis_probe) (lis_pci_dev_t *dev, 
			  const struct lis_pci_device_id *id);
	void _RP (*lis_remove) (lis_pci_dev_t *dev);
	int _RP (*lis_suspend) (lis_pci_dev_t *dev, unsigned int event);
	int _RP (*lis_suspend_late) (lis_pci_dev_t *dev, unsigned int event);
	int _RP (*lis_resume_early) (lis_pci_dev_t *dev);
	int _RP (*lis_resume) (lis_pci_dev_t *dev);
	void _RP (*lis_shutdown) (lis_pci_dev_t *dev);
};
int lis_pci_register_driver(struct lis_pci_driver *lis_driver) _RP __attribute__((weak));
void lis_pci_unregister_driver(struct lis_pci_driver *lis_driver) _RP __attribute__((weak));
